...
func doSet(conn net.Conn, args []interface{}) {
	...
	id := misc.UUId()
	ok = luckydraw.Distribute(id, money, number)
...
func doGet(conn net.Conn, args []interface{}) {
	...
	envelop, ok := luckydraw.TableEnvelopes[id]
	if !ok {
		misc.DEBUG("to read from mongo")
		envelop = readFromMgo(id)
		if envelop != nil {
			luckydraw.TableEnvelopes[id] = envelop
			ok = true
		}
	}
	...
	if ok {
		...
		if opened == nil {
			...
			misc.DEBUG("total envelopes: ", len(luckydraw.TableEnvelopes))
			respCode.Code = 0
			respCode.Message = "SUCCESS"
...
func readFromMgo(id uint64) *luckydraw.M_envelop {
	session, err := mgo.Dial(mongodbAddr)
	if err != nil {
	...
	envelop := new(luckydraw.M_envelop)
...
func storeInMgo(id uint64) {
	if envelop, ok := luckydraw.TableEnvelopes[id]; ok {
		...
		go func() {
			...
			if envelop.Opened == envelop.Size {
				...
				delete(luckydraw.TableEnvelopes, id)
			}
		}()
	}
}
